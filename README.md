...
This repository contains a Vue 3 single-page application (SPA) for the integrations service.

## Prerequisites
- Velocity Chain [Developers Edition](https://github.com/velocity-chain/flux/blob/main/CONTRIBUTING.md)
- Developer [SPA Standard Prerequisites](https://github.com/velocity-chain/flux/blob/main/DeveloperEdition/standards/spa_standards.md)
...
**Note**: This template uses `@velocity-chain/flux_spa_utils` for reusable components, composables, and utilities. See the [spa_utils README](../spa_utils/README.md) for complete documentation on available components (`AutoSaveField`, `AutoSaveSelect`, `ListPageSearch`), composables (`useResourceList`, `useErrorHandler`, `useRoles`), and utilities (`formatDate`, `validationRules`).
...
This template uses components and composables from `@avelocity-chain/flux_spa_utils`:
