<template>
  <v-container>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="integration-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/integrations/new"
          data-automation-id="integration-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(integrations ?? []) as unknown as Integration[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToIntegration"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="integration-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load integrations: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Integrations List Page - Showcase of flux_spa_utils ease of use
 * 
 ...
 * from @velocity-chain/flux_spa_utils. Just provide your API call and you're done!
 */
...
const {
  ...
  queryFn: (params) => api.getIntegrations(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToIntegration(_event: unknown, { item }: { item: Integration }) {
  router.push(`/integrations/${item._id}`)
}
